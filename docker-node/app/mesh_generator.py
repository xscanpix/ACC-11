...
def generate_geo(angle, dstdir, naca1=0, naca2=0, naca3=1, naca4=2, n_nodes=200, n_levels=0):
...
def generate_every_geo(angle_start, angle_stop, n_angles, dstdir="geo", naca1=0, naca2=0, naca3=1, naca4=2, n_nodes=200, n_levels=0):
    ...
    for i in  range(n_angles + 1):
        ...
        generate_geo(angle, dstdir)


# Convert GEO file to MSH file if it does not exist already
def convert_geo_to_msh(filepath, dstdir, gmshbin="/usr/bin/gmsh"):
    if not os.path.exists(dstdir):
        os.mkdir(dstdir)
...
def convert_every_geo_to_msh(srcdir, dstdir, gmshbin="/usr/bin/gmsh"):
...
def convert_msh_to_xml(filepath, dstdir, dolfinconvertpath="/usr/bin/dolfin-convert"):
    if not os.path.exists(dstdir):
        os.mkdir(dstdir)
...
def convert_every_msh_to_xml(srcdir, dstdir, dolfinconvertpath="/usr/bin/dolfin-convert"):
    for filename in os.listdir(srcdir):
        filepath = os.path.abspath("{}/{}".format(srcdir, filename))
        convert_msh_to_xml(filepath, dstdir)


def generate_mesh(angle, gmshbin="/usr/bin/gmsh", geodir="geo", mshdir="msh", naca1=0, naca2=0, naca3=1, naca4=2, n_nodes=200, n_levels=0):
    if not os.path.exists(geodir):
        os.mkdir(geodir)
    if not os.path.exists(mshdir):
        os.mkdir(mshdir)

    geofile = "{}/a{}n{}.geo".format(geodir, angle, n_nodes)
    naca.generate(naca1, naca2, naca3, naca4, angle, n_nodes, geofile)

    temp = geofile.replace(".geo","")
    temp = "{}/r0{}.msh".format(mshdir, temp)
    geo_name = "{}/{}".format(geodir, geofile)
...
def convert_one_msh_xml(srcfilepath, dstdir="xml", dolfinconvertpath="/usr/bin/dolfin-convert"):
    absfilepath = os.path.abspath(srcfilepath)
    absdstdir = os.path.abspath(dstdir)
    basename = ntpath.basename(srcfilepath).replace(".msh", ".xml")
